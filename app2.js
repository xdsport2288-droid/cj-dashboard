...
class CustomMultiSelect {
    constructor(selectElement, defaultText) {
        ...
        this.scrollContent.className = 'custom-multiselect-scroll';
...
function updateTable() {
    ...
    if (window.syncScrollWidth) {
        setTimeout(window.syncScrollWidth, 50);
    }
}
...
if (dynamicTableWrapper) {
    ...
    if (dynamicThead) {
        ...
        const theadResizeObserver = new ResizeObserver(() => {
            const h = dynamicThead.offsetHeight;
            styleEl.textContent = `
                .table-wrapper::-webkit-scrollbar-track:vertical { margin-top: ${h}px !important; }
                .btn-jump-up { height: ${h}px !important; }
            `;
        });
...
async function setupFileSync() {
    ...
    btnSync.addEventListener('click', async () => {
        try {
            ...
            syncStatus.innerHTML = '<span style="color: #3b82f6;">����� - �ڵ� ���� �����...</span>';
...
async function checkFileAndSync() {
    ...
    try {
        ...
        if (file.lastModified > lastModifiedTime) {
            ...
            const syncStatus = document.getElementById('sync-status');
            syncStatus.innerHTML = '<span style="color: #f59e0b;">?? ������ ���� ��...</span>';
            ...
            syncStatus.innerHTML = '<span style="color: #10b981;">? ' + timeStr + ' �ڵ� ���ŵ�</span>';
        }
    } catch (e) {
        console.error('Error monitoring file:', e);
        document.getElementById('sync-status').innerHTML = '<span style="color: #ef4444;">? ���� ���� ����</span>';
    }
}
        ...
        reader.onload = function(e) {
            try {
                ...
                const mappedData = json.map(row => {
                    const carInfoStr = String(row['��������'] || '').trim();
                    const carInfo = carInfoStr ? carInfoStr.split(' ') : [];
                    const reqTon = carInfo.length > 0 ? carInfo[0] : "";
                    const reqCar = carInfo.length > 1 ? carInfo[1] : "";
                    
                    const fare = (row['����'] !== "" && !isNaN(row['����'])) ? parseFloat(row['����']) : 0.0;
                    const extra = (row['�߰���'] !== "" && !isNaN(row['�߰���'])) ? parseFloat(row['�߰���']) : 0.0;
                    const waypoints = (row['����������'] !== "" && !isNaN(row['����������'])) ? parseFloat(row['����������']) : 0.0;
                    const carCount = (row['�������'] !== "" && !isNaN(row['�������'])) ? parseFloat(row['�������']) : 1.0;
                ...
                const timeSpan = document.getElementById('last-updated-time');
                if (timeSpan && window.LAST_UPDATED) {
                    timeSpan.textContent = "�ֱ� ������Ʈ: " + window.LAST_UPDATED;
                }
