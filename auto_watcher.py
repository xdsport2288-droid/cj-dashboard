...
while True:
    ...
    for f in current_files:
        if "~$tmp" in f or "~$" in f: continue  # 엑셀 임시 잠금파일 제외
        mtime = get_mtime(f)
    ...
    if changed:
        ...
        subprocess.run(["git", "pull", "--no-rebase", "-X", "ours"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=NO_WINDOW)
